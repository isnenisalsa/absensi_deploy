...
export const createEmployee = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const finalPassword = password || `P4ssw0rd4ria${nrp}`;

    if (!nrp || !full_name) {
      res.status(400).json({ error: 'NRP dan Nama Lengkap wajib diisi' });
      return;
    }
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      const employee = await tx.employees.create({
        data: {
          nrp,
          full_name,
          pos_id: parseId(pos_id),
          div_id: parseId(div_id),
          dept_id: parseId(dept_id),
          location_id: parseId(location_id),
          mitra_id: finalMitraId,
          default_work_location: default_work_location || 'WFO',
          allow_any_location: !!allow_any_location,
          allowed_locations: {
            create: (location_ids || []).map((locId: any) => ({
...
export const updateEmployee = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.log(`[BACKEND] Updating employee ${id} with data:`, req.body);

    const result = await prisma.$transaction(async (tx: any) => {
      let updateDataUser: any = {};
      // Force role to employee for this specific controller/module
      updateDataUser.role = 'employee';
      if (is_active !== undefined) {
        updateDataUser.is_active = is_active === 'true' || is_active === true;
      }
      if (password) {
        const salt = await bcrypt.genSalt(10);
      ...
      const employee = await tx.employees.update({
        ...
        data: {
          ...
          ...(pos_id !== undefined && { pos_id: parseId(pos_id) }),
          ...(div_id !== undefined && { div_id: parseId(div_id) }),
          ...(dept_id !== undefined && { dept_id: parseId(dept_id) }),
          ...(location_id !== undefined && { location_id: parseId(location_id) }),
          ...(mitra_id !== undefined && !userMitraId && { mitra_id: parseId(mitra_id) }), // Only super can change mitra
    ...
    console.error(`[BACKEND ERROR] Update employee failed:`, error);
